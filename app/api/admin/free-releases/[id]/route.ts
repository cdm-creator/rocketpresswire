...
function identifyRelease(query: any, id: string) {
    return isUuid(id) ? query.eq("id", id) : query.eq("release_id", id)
}
...
export async function GET(request: Request, context: RouteContext) {
    try {
        ...
        const { data, error } = await identifyRelease(
            supabaseAdmin.from("free_releases").select("*"),
            id
        ).maybeSingle()

        if (error) {
            console.error("[admin-free-release-detail] Failed to load release", {
        ...
        return jsonResponse(
            {
                release: normalizeAdminFreeRelease(
        ...
        console.error("[admin-free-release-detail] Server error", {
